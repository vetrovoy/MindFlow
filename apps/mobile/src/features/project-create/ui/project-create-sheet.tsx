...
export function ProjectCreateSheet() {
  ...
  const closeProjectCreate = useMobileAppStore(
    s => s.actions.closeProjectCreate,
  );
  const createProjectFromSheet = useMobileAppStore(
    s => s.actions.createProjectFromSheet,
  );
    ...
    >
      ...
      <SurfaceCard elevated style={styles.card}>
        ...
        <Pressable
          accessibilityRole="button"
          onPress={() => {
            setDraft(d => ({ ...d, isFavorite: !d.isFavorite }));
          }}
          style={[
            styles.favoriteRow,
            {
