...
const repository = createDexieRepositoryBundle({ name: "mindflow-web" });

export const useMindFlowStore = create<MindFlowStore>((set, get) => {
  ...
  const applySnapshot = async () => {
    ...
    const visibleInboxIds = new Set(
      sortTasks(
        getVisibleTasks(snapshot.tasks, snapshot.projects).filter((task) => task.projectId == null)
      ).map((task) => task.id)
    );
