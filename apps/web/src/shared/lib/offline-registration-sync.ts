 ...
 *
 * When a user registers while offline, their account exists only locally.
...
import { getUserDatabaseName } from "@/shared/model/app-storage.config";
...
/**
 * Callback that auth-provider registers so we can update React state
 * after patching the persisted snapshot.
 */
 ...
 * Call once at app startup (e.g., in AuthProvider useEffect).
 */
export function startOfflineRegistrationSync(
  ...
  // Clean up previous instance (React StrictMode safety)
  cleanup?.();
...
async function attemptSync(): Promise<void> {
  ...
  try {
    ...
    // 4. Notify auth-provider to re-render with new session
...
async function uploadLocalData(
  ...
  const dbName = getUserDatabaseName(user.id);
  const localRepo = createDexieRepositoryBundle({ name: dbName });
