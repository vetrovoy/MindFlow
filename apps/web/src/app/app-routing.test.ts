...
import { buildProtectedRouteRedirect, getAuthRedirectTarget } from "./app-routing";

describe("app routing helpers", () => {
  it("builds protected route redirects with search and hash intact", () => {
    expect(buildProtectedRouteRedirect("/search", "?q=archive", "#results")).toBe(
      "/search?q=archive#results"
    );
  });
  ...
  it("resolves only safe in-app auth redirect targets", () => {
    ...
    expect(getAuthRedirectTarget({ from: "https://evil.example" })).toBe("/inbox");
